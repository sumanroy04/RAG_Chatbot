...
from crisis import get_crisis_html_card, get_crisis_response, is_crisis
...
def image_to_data_uri(file_name: str) -> str:
    path = os.path.join(working_dir, "images", file_name)
...
def get_llm_reply(user_input: str, selected_mood: str | None = None, selected_topic: str | None = None) -> tuple[str, bool]:
    ...
    if MAIN_IMPORT_ERROR is not None:
        return f"App import failed from `main.py`: {MAIN_IMPORT_ERROR}", False
...
if not GROQ_API_KEY:
    st.warning(
        "No Groq API key found. Add `GROQ_API_KEY` to `.env` or `config.json`. "
