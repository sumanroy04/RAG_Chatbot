...
# Usage: .\run.ps1
# Requirements: Python venv at backend\.venv, Node modules at frontend\node_modules
...
if (-not (Test-Path $VenvPython)) {
    ...
    Write-Host "      .\.venv\Scripts\pip install -r requirements-py.txt" -ForegroundColor Yellow
    exit 1
}
...
# Copy root .env key into backend src if a backend .env doesn't already have it
$RootEnv    = Join-Path $ProjectRoot ".env"
$BackendEnv = Join-Path $BackendDir "src\.env"
if ((Test-Path $RootEnv) -and (-not (Test-Path $BackendEnv))) {
    Copy-Item $RootEnv $BackendEnv
    Write-Host "ℹ️   Copied root .env to backend\src\.env" -ForegroundColor DarkGray
}
...
$backendCmd = "Set-Location '$BackendDir\src'; & '$VenvUvicorn' main:app --reload --host 127.0.0.1 --port 8000; Read-Host 'Press Enter to close'"
